...
export class LaVerb {
    ...
    public get_lemma_forms(data: ConjugationData, do_linked: boolean): string[] {
        ...
        for (const slot of this.potential_lemma_slots) {
            const lemma_forms = this.get_valid_forms(data.forms.get(linked_prefix + slot));
    ...
    private detect_decl_and_subtypes(args: ArgMap): [ConjType, ConjType | undefined, Set<string>, string, string] {
    ...
    private get_subtype_by_ending(lemma: string, conjtype: string, specified_subtypes: Set<string>, endings_and_subtypes: FormMap): [string, string[]] {
    ...
    private make_perfect_passive(data: ConjugationData) {
        const ppp = data.forms.get("perf_pasv_ptc") || [];
